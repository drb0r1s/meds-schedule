...
doseMedication.post("/take", async (req, res) => {
    ...
    try {
        const doseQueryResult = await DB.dose.setStatus({ dose, status: "taken" });
        if(doseQueryResult.affectedRows) console.log("Row has been updated in Dose table.");
...
doseMedication.post("/missed", async (req, res) => {
    const { dose } = req.body;

    if(dose.status !== "pending") return;

    try {
        const queryResult = await DB.dose.setStatus({ dose, status: "missed" });
